...
build_service() {
    if [ -z "$1" ]; then
        echo -e "${RED}Usage: ./docker-scripts.sh build_service <service_name>${NC}"
        echo -e "${YELLOW}Available services: backend, worker, worker-translation${NC}"
        exit 1
    fi
    echo -e "${GREEN}Building $1...${NC}"
    $DOCKER_COMPOSE build --no-cache "$1"
...
logs_workers() {
    echo -e "${GREEN}Showing logs for both workers...${NC}"
    $DOCKER_COMPOSE logs -f worker worker-translation
}
...
health() {
    ...
    services=("postgres" "redis" "libretranslate" "backend" "worker" "worker-translation")
    container_names=("travel-postgres" "travel-redis" "libretranslate" "travel-backend" "travel-worker" "travel-worker-translation")
...
migrate_db() {
    ...
    if ! $DOCKER_COMPOSE ps postgres | grep -q "Up"; then
        echo -e "${RED}PostgreSQL is not running. Start it first with: ./docker-scripts.sh start${NC}"
        exit 1
    fi
    
    # Check if migration file exists
    if [ ! -f "init-database.sql" ]; then
        echo -e "${RED}Migration file 'init-database.sql' not found!${NC}"
        echo -e "${YELLOW}Make sure the file is in the same directory as this script.${NC}"
        exit 1
    fi
    
    echo -e "${YELLOW}This will create all database tables for your TypeORM entities.${NC}"
    echo -e "${YELLOW}Tables to be created: users, projects, chapters, images, translations, documents, jobs${NC}"
    echo ""
    ...
    echo -e "${BLUE}Copying migration file to container...${NC}"
    docker cp init-database.sql travel-postgres:/tmp/init-database.sql
    ...
    $DOCKER_COMPOSE exec -T postgres psql -U travel -d travel_guides -f /tmp/init-database.sql
    ...
    if [ $? -eq 0 ]; then
        ...
        $DOCKER_COMPOSE exec postgres psql -U travel -d travel_guides -c "\dt"
        
        # Clean up
        docker exec travel-postgres rm /tmp/init-database.sql
        
        echo ""
        echo -e "${GREEN}Database is ready! ✨${NC}"
        echo ""
        echo -e "${YELLOW}⚠️  IMPORTANT: Update your TypeORM config to set 'synchronize: false'${NC}"
        echo -e "${YELLOW}This prevents automatic schema changes in production.${NC}"
    else
...
db_schema() {
    if [ -z "$1" ]; then
        echo -e "${RED}Usage: ./docker-scripts.sh db_schema <table_name>${NC}"
        echo -e "${YELLOW}Available tables: users, projects, chapters, images, translations, documents, jobs${NC}"
        exit 1
    fi
    echo -e "${GREEN}Schema for table: $1${NC}"
    $DOCKER_COMPOSE exec postgres psql -U travel -d travel_guides -c "\d $1"
}

# Database shell access
db_shell() {
    echo -e "${GREEN}Opening PostgreSQL shell...${NC}"
    echo -e "${YELLOW}Database: travel_guides, User: travel${NC}"
    echo -e "${YELLOW}Type \q to exit${NC}"
    echo ""
    $DOCKER_COMPOSE exec postgres psql -U travel -d travel_guides
}
...
db_reset() {
    ...
    if [ "$response" = "yes" ]; then
        echo -e "${RED}Dropping schema...${NC}"
        $DOCKER_COMPOSE exec postgres psql -U travel -d travel_guides -c "DROP SCHEMA public CASCADE; CREATE SCHEMA public;"
        echo -e "${GREEN}Database reset complete. Run './docker-scripts.sh migrate_db' to recreate tables.${NC}"
    else
...
help() {
    ...
    echo ""
    echo -e "${BLUE}Database:${NC}"
    echo "  migrate_db               - 🆕 Run database migration (create tables)"
    echo "  db_tables                - 🆕 List all database tables"
    echo "  db_schema <table>        - 🆕 Show schema for specific table"
    echo "  db_shell                 - 🆕 Open PostgreSQL interactive shell"
    echo "  db_reset                 - 🆕 Reset database (DROP ALL TABLES)"
    echo "  backup_db                - Create database backup"
    echo "  restore_db <file>        - Restore database from backup"
    echo ""
    echo -e "${BLUE}Redis:${NC}"
    ...
    echo -e "${BLUE}Container Access:${NC}"
    echo "  exec_backend <cmd>       - Execute command in backend container"
    echo "  exec_worker <cmd>        - Execute command in worker"
    echo "  exec_worker_trans <cmd>  - Execute command in translation worker"
    echo ""
    echo -e "${BLUE}Cleanup:${NC}"
    echo "  clean                    - Remove all containers, volumes, and images"
    ...
    echo "  help                     - Show this help message"
    echo ""
    echo -e "${GREEN}Database Migration Examples:${NC}"
    echo "  ./docker-scripts.sh migrate_db           # Create all tables"
    echo "  ./docker-scripts.sh db_tables            # List tables"
    echo "  ./docker-scripts.sh db_schema users      # Show users table structure"
    echo "  ./docker-scripts.sh db_shell             # Open psql shell"
    echo ""
    ...
    echo -e "${YELLOW}Services available:${NC}"
    echo "  • backend              - Main NestJS API"
    echo "  • worker               - Document generation worker"
    echo "  • worker-translation   - Document translation worker"
    echo "  • postgres             - PostgreSQL database"
    echo "  • redis                - Redis cache/queue"
    echo "  • libretranslate       - Translation service"
